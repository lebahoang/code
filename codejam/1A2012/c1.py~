...
def solve(L,R,total,last_min):
    ...
    for i in range(len(L)):
        L[i] = (L[i][0],L[i][0]*min_v+L[i][1],L[i][2])
    for i in range(len(R)):
        R[i] = (R[i][0],R[i][0]*min_v+R[i][1],R[i][2])
    ...
    if len(R) ==0:
        ...
        t.pop(a)
        t1.insert(i+1,L[a])
        if store.get(("L",(it[2] for it in t),"R",(it[2] for it in t1))) ==None: 
            rs1,last_min = solve(t,t1,total,last_min)
        else:
            rs1,last_min = store[("L",(it[2] for it in t),"R",(it[2] for it in t1))]
        ...
        if store.get(("L",(it[2] for it in t),"R",(it[2] for it in t1))) ==None:
            ...
            rs2,last_min = store[("L",(it[2] for it in t),"R",(it[2] for it in t1))]
        if rs1 == -1 or rs2 ==-1: total =-1
        else: total = max(rs1,rs2)
        store[("L",(it[2] for it in L),"R",(it[2] for it in R))] = (total,last_min)
        return (total,last_min)
    rs1 = rs2 = 0
    if L[a][1] + 5 <= R[0][1]:
        stop=1
...
for _ in range(testcase):
    ...
    for _ in range(n):
        ...
        if lane == "L":
            L.append((float(S),float(P),co))
        elif lane == "R":
            R.append((float(S),float(P),co))
        co +=1
    for i in range(len(L)-1):
